...
impl Base {
    pub fn parseTagAssetID(tllv: &FPSServerTLLV, spcContainer: &mut FPSServerSPCContainer) -> Result<()> {
        ...
        spcContainer.spcData.spcAssetInfo.id =
            match String::from_utf8(tllv.value.to_owned()) {
                Ok(r) => r,
                Err(e) => String::from_utf8(e.as_bytes()[0..e.utf8_error().valid_up_to()].to_vec()).unwrap_or_default(),
            };
