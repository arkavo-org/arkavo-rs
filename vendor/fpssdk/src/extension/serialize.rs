...
impl Serialize for FPSResult {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
        ...
        if self.status == FPSStatus::noErr {
            ...
            playbackStateMap.insert(CREATION_DATE_STR.to_string(), self.resultAssetInfo.playInfo.date.into());
            playbackStateMap.insert(STATE_STR.to_string(), self.resultAssetInfo.playInfo.playbackState.into());
            playbackStateMap.insert(SESSION_ID_STR.to_string(), self.resultAssetInfo.playInfo.playbackId.into());
            ...
            match self.supportedSecurityLevel {
                ...
                _ => {
                    ...
                    let securityLevelHex: String = format!("{:016X}", self.supportedSecurityLevel.unwrap_or_default());
            ...
            capabilitiesMap.insert(HDCP_STR.to_string(), (capabilitiesLowBits & FPS_CAPABILITY_HDCP_TYPE1_ENFORCEMENT_SUPPORTED != 0).into());
            capabilitiesMap.insert(OFFLINE_KEY_STR.to_string(), (capabilitiesLowBits & FPS_CAPABILITY_OFFLINE_KEY_SUPPORTED != 0).into());
            capabilitiesMap.insert(SECURE_INVALIDATION_STR.to_string(), (capabilitiesLowBits & FPS_CAPABILITY_CHECK_IN_SUPPORTED != 0).into());
            capabilitiesMap.insert(OFFLINE_KEY_2_STR.to_string(), (capabilitiesLowBits & FPS_CAPABILITY_OFFLINE_KEY_V2_SUPPORTED != 0).into());
            capabilitiesMap.insert(SECURITY_LEVEL_BASELINE_STR.to_string(), (capabilitiesLowBits & FPS_CAPABILITY_SECURITY_LEVEL_BASELINE_SUPPORTED != 0).into());
            capabilitiesMap.insert(SECURITY_LEVEL_MAIN_STR.to_string(), (capabilitiesLowBits & FPS_CAPABILITY_SECURITY_LEVEL_MAIN_SUPPORTED != 0).into());
            ...
            if self.deviceIdentity.isDeviceIdentitySet {
                ...
                // FPDI Version
                deviceIdentityMap.insert(FPDI_VERSION_STR.to_string(), self.deviceIdentity.fpdiVersion.into());

                // Device Class
                deviceIdentityMap.insert(DEVICE_CLASS_STR.to_string(), self.deviceIdentity.deviceClass.into());
            ...
            if let Some(vmDeviceInfo) = self.vmDeviceInfo.as_ref() {
                ...
                vmDeviceInfoMap.insert(HOST_DEVICE_CLASS_STR.to_string(), (vmDeviceInfo.hostDeviceClass as u32).into());
                ...
                vmDeviceInfoMap.insert(HOST_VM_PROTOCOL_VERSION.to_string(), vmDeviceInfo.hostVMProtocolVersion.into());

                // Print Guest VM Information
                vmDeviceInfoMap.insert(GUEST_DEVICE_CLASS_STR.to_string(), (vmDeviceInfo.guestDeviceClass as u32).into());
                ...
                vmDeviceInfoMap.insert(GUEST_VM_PROTOCOL_VERSION.to_string(), vmDeviceInfo.guestVMProtocolVersion.into());
            ...
            if self.offlineSyncData.version > 0 {
                ...
                // Duration to Expiry
                offlineSyncMap.insert(DURATION_TO_EXPIRY_STR.to_string(), self.offlineSyncData.durationToRentalExpiry.into());

                if self.offlineSyncData.version >= 2 {
                    // Server Challenge
                    offlineSyncMap.insert(SERVER_CHALLENGE_STR.to_string(), self.offlineSyncData.syncServerChallenge.into());

                    // Flags
                    let mut flagsMap = Map::new();
                    flagsMap.insert(REPORT_STR.to_string(), (self.offlineSyncData.syncFlags & KD_SYNC_SPC_FLAG_REPORT != 0).into());
                    flagsMap.insert(SECURE_INVALIDATION_STR.to_string(), (self.offlineSyncData.syncFlags & KD_SYNC_SPC_FLAG_SECURE_INVALIDATION != 0).into());
                    flagsMap.insert(SECURE_INVALIDATION_ALL_STR.to_string(), (self.offlineSyncData.syncFlags & KD_SYNC_SPC_FLAG_SECURE_INVALIDATION_ALL != 0).into());
                    flagsMap.insert(TITLE_ID_VALID_STR.to_string(), (self.offlineSyncData.syncFlags & KD_SYNC_SPC_FLAG_TITLEID_VALID != 0).into());
                    flagsMap.insert(SUCCESS_STR.to_string(), (self.offlineSyncData.syncFlags & KD_SYNC_SPC_FLAG_SUCCESS != 0).into());
                    flagsMap.insert(OBJ_NOT_FOUND_STR.to_string(), (self.offlineSyncData.syncFlags & KD_SYNC_SPC_FLAG_OBJ_NOT_FOUND != 0).into());
                    flagsMap.insert(OBJ_EXPIRED_STR.to_string(), (self.offlineSyncData.syncFlags & KD_SYNC_SPC_FLAG_OBJ_EXPIRED != 0).into());
                    ...
                    if (self.offlineSyncData.syncFlags & KD_SYNC_SPC_FLAG_TITLEID_VALID) !=0 {
                        let hextTitleId: String = self.offlineSyncData.syncTitleId.encode_hex_upper();
                    ...
                    // Records Invalidated
                    offlineSyncMap.insert(RECORDS_INVALIDATED_STR.to_string(), self.offlineSyncData.recordsDeleted.into());

                    // Array of Invalidated Stream IDs
                    let mut streamIDs: Vec<String> = Vec::new();
                    if self.offlineSyncData.recordsDeleted > 0 && !self.offlineSyncData.deletedStreamIDs.is_empty() {
                        for i in 0..self.offlineSyncData.recordsDeleted {
                            let index = i * FPS_OFFLINE_CONTENTID_LENGTH;
                            let hexContentID = self.offlineSyncData.deletedStreamIDs[index..(index + FPS_OFFLINE_CONTENTID_LENGTH)].to_vec().encode_hex_upper();
            ...
            if self.operationType == FPSOperationType::createCKC && !self.isCheckIn && !self.ckc.is_empty() {
                //CKC
...
impl Serialize for FPSResults {
    ...
    {
        let structLen = 1;
        let mut structSerializer = serializer.serialize_struct(extension_constants::FAIRPLAY_STREAMING_RESPONSE_STR, structLen)?; 

        if !self.resultPtr.is_empty() && self.resultPtr.first().unwrap().operationType == FPSOperationType::getClientInfo {
    ...
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
        where
            S: serde::Serializer {
                ...
                structSerializer.serialize_field(extension_constants::FAIRPLAY_STREAMING_RESPONSE_STR, &self.results)?;
