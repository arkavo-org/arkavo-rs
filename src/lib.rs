...
mod tests {
    ...
    impl NanoTDFTests {
        ...
        fn test_policy_with_salt() -> Result<(), Box<dyn Error>> {
            // This test verifies that the parser correctly handles NanoTDFs with salt in policy
            // Using version M (0x4D) and adding salt field after policy data
            let hex_string = "\
                4c 31 4d 01 0e 6b 61 73 2e 76 69 72 74 72 75 2e 63 6f 6d 80\
                00 01 15 6b 61 73 2e 76 69 72 74 72 75 2e 63 6f 6d 2f 70\
                6f 6c 69 63 79 10 aa bb cc dd ee ff 00 11 22 33 44 55 66 77 88 99 b5 e4 13 a6 02 11 e5 f1 7b 22 34 a0 cd 3f 36\
            ...
            println!("Testing policy with salt, parsing hex data...");
            let bytes = hex::decode(hex_string.replace(" ", "").replace("\n", ""))?;
            println!("Hex data decoded, length: {}", bytes.len());
            let mut parser = BinaryParser::new(&*bytes);
            let header = parser.parse_header()?;
            
            println!("Header parsed successfully");
            // Check that the salt is correctly parsed from the policy
            assert!(header.get_policy().get_salt().is_some());
            let salt = header.get_policy().get_salt().as_ref().unwrap();
            assert_eq!(salt.len(), 16);
            assert_eq!(&salt[0..4], &[0xaa, 0xbb, 0xcc, 0xdd]);
            
            Ok(())
        }

        fn test_spec_example_binary_parser() -> Result<(), Box<dyn Error>> {
            // Version L (0x4C) NanoTDF example without salt
            let hex_string = "\
            ...
            println!("Testing legacy NanoTDF (version L) without salt...");
            let bytes = hex::decode(hex_string.replace(" ", "").replace("\n", ""))?;
            println!("Hex data decoded, length: {}", bytes.len());
            let mut parser = BinaryParser::new(&*bytes);
            let header = parser.parse_header()?;
            ...
            println!("Header parsed successfully");
            // Verify this is version L
            assert_eq!(header.version.len(), 1);
            assert_eq!(header.version[0], 0x4C);
            
            // Verify no salt in version L
            assert!(header.get_policy().get_salt().is_none());
